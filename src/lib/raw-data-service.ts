...
export async function getRawDataByChatId(chatId: string, includeRawJson: boolean = false) {
  ...
  return {
    ...
    traceabilityChain: buildTraceabilityChain(chatLog.RawImportData.dataLinks)
  }
}
...
export async function getTraceabilityChain(rawDataId: string) {
  ...
  return {
    ...
    chain: buildTraceabilityChain(rawData.dataLinks)
  }
}
...
export async function getAllRawImports(options: {
  ...
  return {
    items: imports.map(imp => ({
      ...
      chatLog: imp.chatLog,
      linkCount: imp._count.dataLinks
    })),
    pagination: {
      page,
      pageSize: limit,
