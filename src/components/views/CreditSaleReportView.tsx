...
function CreditSaleReportSkeleton() {
  return (
...
export function CreditSaleReportView() {
  ...
  const fetchReport = useCallback(async (from?: string, to?: string, searchVal?: string) => {
    try {
      ...
      if (!json.success) {
        toast.error('Failed to fetch credit sale report')
        return
      }
  ...
  const outstanding = data.reduce((sum, s) => sum + (s.isPaid ? 0 : s.grandTotal ?? 0), 0)

  if (loading && !filtering) {
    return <CreditSaleReportSkeleton />
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Summary of all credit sale transactions with payment status
          </p>
        </div>
      </div>
      ...
      <div className="print-only hidden text-center space-y-1 mb-4">
        <h3 className="text-lg font-bold">Credit Sale Report</h3>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-orange-600 tabular-nums">{formatCurrency(grandTotal)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-rose-600 tabular-nums">{formatCurrency(outstanding)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-0">
          {data.length === 0 ? (
            <div className="p-12 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm font-medium text-foreground">No credit sales found</p>
              <p className="text-xs text-muted-foreground mt-1">
                No credit sale records match your filter criteria
              </p>
            </div>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/80 hover:bg-slate-50/80">
                    ...
                    <TableHead className="text-xs font-semibold text-foreground text-right hidden lg:table-cell">Balance</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {data.map((sale) => (
                    <TableRow key={sale.id} className="group">
                      <TableCell className="text-xs font-medium font-mono text-orange-700">
                        {sale.saleNo}
                      </TableCell>
                        ...
                        <Badge
                          ...
                          className={
                            sale.isPaid
                              ...
                              : 'bg-rose-100 text-rose-700 hover:bg-rose-100 text-[10px] px-1.5 py-0'
                          }
                        >
                      ...
                      <TableCell className="text-xs text-right tabular-nums hidden lg:table-cell">
                        <span className={sale.isPaid ? 'text-muted-foreground' : 'text-rose-600 font-semibold'}>
                          {sale.isPaid ? formatCurrency(0) : formatCurrency(sale.grandTotal)}
                        </span>
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            <div className="border-t bg-slate-50/80 p-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm font-bold text-orange-600 tabular-nums">{formatCurrency(grandTotal)}</p>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm font-bold text-rose-600 tabular-nums">{formatCurrency(outstanding)}</p>
                </div>
              </div>
            </div>
          )}
