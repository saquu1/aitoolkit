...
function ReceiptEntrySkeleton() {
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3"><Skeleton className="h-9 w-9 rounded-lg" /><Skeleton className="h-7 w-36" /></div>
      <Card className="py-0 gap-0"><CardContent className="p-6"><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-10 w-full" />)}</div><div className="flex gap-3 mt-6"><Skeleton className="h-10 w-32" /><Skeleton className="h-10 w-36" /></div></CardContent></Card>
...
export function ReceiptEntryView() {
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      const res = await fetch('/api/transactions?transType=RECEIPT&limit=20')
  ...
  function resetForm() { setFormDate(format(new Date(), 'yyyy-MM-dd')); setFromAccountId(''); setBankId(''); setFormAmount(''); setFormRef(''); setFormComments('') }

  async function handleSave(andNew = false) {
    if (!formDate) { toast.error('Date is required'); return }
    if (!fromAccountId) { toast.error('Select received-from account'); return }
    if (!bankId) { toast.error('Select a bank'); return }
    if (fromAccountId === bankId) { toast.error('Source and bank cannot be the same'); return }
    ...
    try {
      ...
      const debitRes = await fetch('/api/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transDate: formDate, accountId: Number(bankId), debit: amount, credit: 0, refNo: formRef.trim() || undefined, comments: formComments.trim() || undefined, transType: 'RECEIPT' }) })
      // Credit (money leaving source)
      const creditRes = await fetch('/api/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transDate: formDate, accountId: Number(fromAccountId), debit: 0, credit: amount, refNo: formRef.trim() || undefined, comments: formComments.trim() || undefined, transType: 'RECEIPT' }) })

      const debitJson = await debitRes.json()
      const creditJson = await creditRes.json()
      if (!debitJson.success || !creditJson.success) { toast.error(debitJson.error || creditJson.error || 'Failed'); return }

      toast.success(`Receipt of ${formatCurrency(amount)} saved`)
      resetForm()
      fetchTransactions()
  ...
  if (loading) return <ReceiptEntrySkeleton />

  const availableFromAccounts = allAccounts.filter((a) => a.id !== Number(bankId))

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="h-9 w-9 rounded-lg bg-teal-100 flex items-center justify-center"><ArrowUpCircle className="h-5 w-5 text-teal-600" /></div>
        <div><h2 className="text-xl font-bold tracking-tight text-foreground">Receipt Entry</h2><p className="text-xs text-muted-foreground">Record receipts into bank from any source</p></div>
      </div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label htmlFor="rcp-date">Date <span className="text-rose-500">*</span></Label>
              <div className="relative"><CalendarDays className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input id="rcp-date" type="date" value={formDate} onChange={(e) => setFormDate(e.target.value)} className="pl-9" /></div>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="rcp-amount">Amount (PKR) <span className="text-rose-500">*</span></Label>
              <Input id="rcp-amount" type="number" min="0" step="1" placeholder="0" value={formAmount} onChange={(e) => setFormAmount(e.target.value)} className="text-right tabular-nums" />
            </div>
            <div className="space-y-1.5">
              <Label>Received From <span className="text-rose-500">*</span></Label>
              <Select value={fromAccountId} onValueChange={setFromAccountId}><SelectTrigger className="w-full"><SelectValue placeholder="Select source account" /></SelectTrigger><SelectContent>{availableFromAccounts.map((a) => <SelectItem key={a.id} value={String(a.id)}>{a.aname}</SelectItem>)}</SelectContent></Select>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5"><Label htmlFor="rcp-ref">Reference No</Label><Input id="rcp-ref" placeholder="Optional" value={formRef} onChange={(e) => setFormRef(e.target.value)} /></div>
            <div className="space-y-1.5"><Label htmlFor="rcp-comments">Comments</Label><Textarea id="rcp-comments" placeholder="Optional notes..." value={formComments} onChange={(e) => setFormComments(e.target.value)} rows={3} /></div>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 mt-6">
            <Button onClick={() => handleSave(false)} disabled={saving} className="bg-teal-500 hover:bg-teal-600 text-white shadow-sm">{saving && <Loader2 className="h-4 w-4 mr-1.5 animate-spin" />}Save</Button>
            <Button onClick={() => handleSave(true)} disabled={saving} variant="outline" className="border-teal-300 text-teal-700 hover:bg-teal-50">{saving && <Loader2 className="h-4 w-4 mr-1.5 animate-spin" />}<Plus className="h-4 w-4 mr-1.5" />Save &amp; New</Button>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2"><h3 className="text-sm font-semibold text-foreground">Recent Receipts</h3><Badge variant="secondary" className="bg-teal-100 text-teal-700 border-teal-200 text-[10px] font-semibold tabular-nums">{transactions.length}</Badge></div>
        {transactions.length === 0 ? (
          <Card className="py-0 gap-0"><CardContent className="p-8 flex flex-col items-center justify-center text-center"><div className="h-12 w-12 rounded-full bg-teal-50 flex items-center justify-center mb-3"><ArrowUpCircle className="h-6 w-6 text-teal-400" /></div><p className="text-sm font-medium text-foreground">No receipts yet</p><p className="text-xs text-muted-foreground mt-1">Create your first receipt entry above.</p></CardContent></Card>
        ) : (
