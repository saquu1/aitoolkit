...
export function PaymentEntryView() {
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      const res = await fetch('/api/transactions?transType=PAYMENT&limit=20')
  ...
  async function handleSave(andNew = false) {
    ...
    if (!accountId) { toast.error('Select a payee account'); return }
    if (bankId === accountId) { toast.error('Bank and payee cannot be the same'); return }
    ...
    try {
      ...
      const creditRes = await fetch('/api/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transDate: formDate, accountId: Number(bankId), debit: 0, credit: amount, refNo: formRef.trim() || undefined, comments: formComments.trim() || undefined, transType: 'PAYMENT' }) })
      // Debit (payee receives)
      const debitRes = await fetch('/api/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transDate: formDate, accountId: Number(accountId), debit: amount, credit: 0, refNo: formRef.trim() || undefined, comments: formComments.trim() || undefined, transType: 'PAYMENT' }) })
      ...
      toast.success(`Payment of ${formatCurrency(amount)} saved`)
      resetForm()
      fetchTransactions()
  ...
  if (loading) return <PaymentEntrySkeleton />

  const availableAccounts = allAccounts.filter((a) => a.id !== Number(bankId))

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="h-9 w-9 rounded-lg bg-amber-100 flex items-center justify-center"><ArrowDownCircle className="h-5 w-5 text-amber-600" /></div>
        <div><h2 className="text-xl font-bold tracking-tight text-foreground">Payment Entry</h2><p className="text-xs text-muted-foreground">Record payments to payees from bank</p></div>
      </div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <div className="relative"><CalendarDays className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input id="pay-date" type="date" value={formDate} onChange={(e) => setFormDate(e.target.value)} className="pl-9" /></div>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="pay-amount">Amount (PKR) <span className="text-rose-500">*</span></Label>
              <Input id="pay-amount" type="number" min="0" step="1" placeholder="0" value={formAmount} onChange={(e) => setFormAmount(e.target.value)} className="text-right tabular-nums" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <Label>Payee Account <span className="text-rose-500">*</span></Label>
              <Select value={accountId} onValueChange={setAccountId}><SelectTrigger className="w-full"><SelectValue placeholder="Select payee" /></SelectTrigger><SelectContent>{availableAccounts.map((a) => <SelectItem key={a.id} value={String(a.id)}>{a.aname}</SelectItem>)}</SelectContent></Select>
            </div>
            <div className="space-y-1.5"><Label htmlFor="pay-ref">Reference No</Label><Input id="pay-ref" placeholder="Optional" value={formRef} onChange={(e) => setFormRef(e.target.value)} /></div>
            <div className="space-y-1.5"><Label htmlFor="pay-comments">Comments</Label><Textarea id="pay-comments" placeholder="Optional notes..." value={formComments} onChange={(e) => setFormComments(e.target.value)} rows={3} /></div>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 mt-6">
            <Button onClick={() => handleSave(false)} disabled={saving} className="bg-amber-500 hover:bg-amber-600 text-white shadow-sm">{saving && <Loader2 className="h-4 w-4 mr-1.5 animate-spin" />}Save</Button>
            <Button onClick={() => handleSave(true)} disabled={saving} variant="outline" className="border-amber-300 text-amber-700 hover:bg-amber-50">{saving && <Loader2 className="h-4 w-4 mr-1.5 animate-spin" />}<Plus className="h-4 w-4 mr-1.5" />Save &amp; New</Button>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2"><h3 className="text-sm font-semibold text-foreground">Recent Payments</h3><Badge variant="secondary" className="bg-amber-100 text-amber-700 border-amber-200 text-[10px] font-semibold tabular-nums">{transactions.length}</Badge></div>
        {transactions.length === 0 ? (
          <Card className="py-0 gap-0"><CardContent className="p-8 flex flex-col items-center justify-center text-center"><div className="h-12 w-12 rounded-full bg-amber-50 flex items-center justify-center mb-3"><ArrowDownCircle className="h-6 w-6 text-amber-400" /></div><p className="text-sm font-medium text-foreground">No payments yet</p><p className="text-xs text-muted-foreground mt-1">Create your first payment entry above.</p></CardContent></Card>
        ) : (
