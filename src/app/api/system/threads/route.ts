...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'cleanup-idle') {
      // Kill ALL idle bash/su sessions (aggressive cleanup)
      let killedCount = 0
      let freedThreads = 0

      try {
        // Get current process info to protect
        const currentPid = process.pid

        // Kill all su processes running bash
        const { stdout: suPids } = await execAsync('pgrep su 2>/dev/null || true')
        const suPidList = suPids.trim().split('\n').filter(Boolean)
        
        for (const pid of suPidList) {
          try {
            const pidNum = parseInt(pid, 10)
            // Get thread count before killing
            const { stdout: threadInfo } = await execAsync(`ps -o nlwp= -p ${pid} 2>/dev/null || echo 1`)
            const threads = parseInt(threadInfo.trim(), 10) || 1
            
            await execAsync(`kill -9 ${pid} 2>/dev/null || true`)
            killedCount++
            freedThreads += threads
          } catch (e) {}
        }

        // Kill idle bash processes (no children)
        const { stdout: bashPids } = await execAsync('pgrep bash 2>/dev/null || true')
        const bashPidList = bashPids.trim().split('\n').filter(Boolean)

        for (const pid of bashPidList) {
          try {
            const pidNum = parseInt(pid, 10)
            
            // Skip current process and its ancestors
            if (pidNum === currentPid) continue
            
            // Check if this bash has any children (active processes)
            const { stdout: children } = await execAsync(`pgrep -P ${pid} 2>/dev/null || true`)
            const childCount = children.trim().split('\n').filter(Boolean).length
      ...
      return NextResponse.json({
        success: true,
        message: `Cleaned up ${killedCount} idle processes, freed ~${freedThreads} threads`,
        killedCount,
        freedThreads,
        stats
      })
