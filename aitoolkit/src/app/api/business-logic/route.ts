...
// ═══════════════════════════════════════════════════════════════════════════
// SAFE MODEL ACCESS WRAPPER
// Many Prisma models referenced below don't exist in the schema yet.
// This wrapper returns graceful fallbacks instead of crashing.
// ═══════════════════════════════════════════════════════════════════════════

type SafeDb = typeof db & Record<string, any>;

const MISSING_MODELS = [
  'businessRule', 'businessRuleVersion', 'stateMachine', 'policy',
  'decisionTable', 'ruleExecutionLog', 'ruleConflict', 'businessProcess',
  'ruleTemplate',
] as const;

const missingSet = new Set<string>(MISSING_MODELS);

function safeDb(model: string): any {
  if (missingSet.has(model)) {
    return {
      findMany: async () => [],
      findFirst: async () => null,
      findUnique: async () => null,
      count: async () => 0,
      create: async (data: any) => ({ id: `mock_${Date.now()}`, ...data }),
      update: async (args: any) => ({ id: args.where?.id || 'unknown', ...args.data }),
      updateMany: async (args: any) => ({ count: 0 }),
      delete: async (args: any) => ({ count: 0 }),
...
async function extractIfThenRules(projectId: string) {
  ...
  for (const rule of extractedRules) {
    try {
      const existing = await safeDb("businessRule").findFirst({
        where: { projectId, ruleName: rule.name }
      });

      if (existing) {
        // Create new version
        await safeDb("businessRuleVersion").create({
          data: {
            businessRuleId: existing.id,
            version: (await safeDb("businessRuleVersion").count({ where: { businessRuleId: existing.id } })) + 1,
        ...
        await safeDb("businessRule").update({
        ...
        await safeDb("businessRule").create({
          data: {
            projectId,
            ruleName: rule.name,
            ruleType: rule.type,
...
async function getBusinessRules(projectId: string, filters?: any) {
  ...
  const rules = await safeDb("businessRule").findMany({
    where,
...
async function storeBusinessRule(projectId: string, rule: any) {
  const created = await safeDb("businessRule").create({
    data: {
      projectId,
      ruleName: rule.name,
...
async function updateBusinessRule(projectId: string, ruleId: string, updates: any) {
  const existing = await safeDb("businessRule").findUnique({
  ...
  // Create new version
  const versionCount = await safeDb("businessRuleVersion").count({
  ...
  // Update rule
  const updated = await safeDb("businessRule").update({
    where: { id: ruleId, projectId },
    data: {
      ...updates,
      version: versionCount + 1,
...
async function deleteBusinessRule(projectId: string, ruleId: string) {
  await safeDb("businessRule").delete({ where: { id: ruleId, projectId } });
...
async function convertValidationToRule(projectId: string, validation: any) {
  ...
  const rule = await safeDb("businessRule").create({
    data: {
      projectId,
      ruleName,
      ruleType: 'validation',
...
async function getValidationRules(projectId: string, viewName?: string) {
  ...
  const rules = await safeDb("businessRule").findMany({ where, orderBy: { createdAt: 'desc' } });
  return NextResponse.json({
    success: true,
...
async function getStateTransitions(projectId: string, tableName: string) {
  const stateMachine = await safeDb("stateMachine").findFirst({
...
async function storeStateMachine(projectId: string, stateMachine: any) {
  const created = await safeDb("stateMachine").create({
    data: {
      projectId,
...
async function createPolicy(projectId: string, policy: any) {
  const created = await safeDb("policy").create({
    data: {
      projectId,
...
async function updatePolicy(projectId: string, policyId: string, updates: any) {
  const current = await safeDb("policy").findUnique({ where: { id: policyId, projectId } });
  if (!current) {
    return NextResponse.json({ error: 'Policy not found' }, { status: 404 });
  }

  const updated = await safeDb("policy").update({
...
async function getPolicies(projectId: string, filters?: any) {
  ...
  const policies = await safeDb("policy").findMany({ where, orderBy: { createdAt: 'desc' } });
...
async function evaluatePolicy(projectId: string, policyId: string, context: any) {
  const policy = await safeDb("policy").findUnique({ where: { id: policyId, projectId } });
  if (!policy) {
    return NextResponse.json({ error: 'Policy not found' }, { status: 404 });
  }
  ...
  // Log execution
  await safeDb("ruleExecutionLog").create({
    data: {
      projectId,
      entityType: 'policy',
      entityId: policyId,
...
async function deletePolicy(projectId: string, policyId: string) {
  await safeDb("policy").delete({ where: { id: policyId, projectId } });
...
async function executeRule(projectId: string, ruleId: string, data: any) {
  const rule = await safeDb("businessRule").findUnique({ where: { id: ruleId, projectId } });
  if (!rule) {
    return NextResponse.json({ error: 'Rule not found' }, { status: 404 });
  }
...
async function getExecutionHistory(projectId: string, filters?: any) {
  ...
  const logs = await safeDb("ruleExecutionLog").findMany({
    where,
    orderBy: { createdAt: 'desc' },
    take: filters?.limit || 100,
  });
...
async function createDecisionTable(projectId: string, decisionTable: any) {
  const created = await safeDb("decisionTable").create({
    data: {
      projectId,
...
async function updateDecisionTable(projectId: string, tableId: string, updates: any) {
  const updated = await safeDb("decisionTable").update({
...
async function getDecisionTables(projectId: string) {
  const tables = await safeDb("decisionTable").findMany({
    where: { projectId },
    orderBy: { createdAt: 'desc' },
  });
...
async function evaluateDecisionTable(projectId: string, tableId: string, input: any) {
  const table = await safeDb("decisionTable").findUnique({
    where: { id: tableId, projectId },
  });
...
async function convertRulesToDecisionTable(projectId: string, ruleIds: string[]) {
  ...
  const decisionTable = await safeDb("decisionTable").create({
    data: {
      projectId,
...
async function createTestFromRule(projectId: string, ruleId: string) {
  const rule = await safeDb("businessRule").findUnique({
    where: { id: ruleId, projectId },
  });
...
async function detectRuleConflicts(projectId: string) {
  ...
  for (const conflict of conflicts) {
    await safeDb("ruleConflict").create({
      data: {
        projectId,
...
async function resolveConflict(projectId: string, conflictId: string, resolution: any) {
  const updated = await safeDb("ruleConflict").update({
...
async function getRuleImpact(projectId: string, ruleId: string) {
  ...
  // Find related rules
  const allRules = await safeDb("businessRule").findMany({
  ...
  const processes = await safeDb("businessProcess").findMany({
    where: { projectId },
  });
...
async function getRuleHistory(projectId: string, ruleId: string) {
  const versions = await safeDb("businessRuleVersion").findMany({
    where: { businessRuleId: ruleId },
...
async function rollbackRule(projectId: string, ruleId: string, version: number) {
  const targetVersion = await safeDb("businessRuleVersion").findFirst({
...
async function compareRuleVersions(projectId: string, ruleId: string, version1: number, version2: number) {
  const v1 = await safeDb("businessRuleVersion").findFirst({
    where: { businessRuleId: ruleId, version: version1 },
  });

  const v2 = await safeDb("businessRuleVersion").findFirst({
...
async function createRuleTemplate(projectId: string, template: any) {
  const created = await safeDb("ruleTemplate").create({
    data: {
      projectId,
      name: template.name,
...
async function getRuleTemplates(projectId: string, category?: string) {
  ...
  const templates = await safeDb("ruleTemplate").findMany({
    where,
    orderBy: { createdAt: 'desc' },
  });
...
async function instantiateTemplate(projectId: string, templateId: string, params: any) {
  const template = await safeDb("ruleTemplate").findUnique({
    where: { id: templateId },
  });
...
async function createBusinessProcess(projectId: string, process: any) {
  const created = await safeDb("businessProcess").create({
    data: {
      projectId,
...
async function mapRulesToProcess(projectId: string, processId: string, ruleMappings: any) {
  const process = await safeDb("businessProcess").findUnique({
  ...
  const updated = await safeDb("businessProcess").update({
    where: { id: processId },
    data: {
...
async function exportRules(projectId: string, format: string, filters?: any) {
  ...
  const rules = await safeDb("businessRule").findMany({ where });
...
async function exportPolicies(projectId: string, format: string) {
  const policies = await safeDb("policy").findMany({ where: { projectId } });
...
async function getRuleAnalytics(projectId: string) {
  const [rules, executions, tests] = await Promise.all([
    safeDb("businessRule").findMany({ where: { projectId }, select: { ruleType: true, severity: true } }),
    safeDb("ruleExecutionLog").findMany({ where: { projectId }, select: { success: true, executionTime: true } }),
...
async function getCoverageReport(projectId: string) {
  const [rules, procedures, cshtmlViews] = await Promise.all([
    safeDb("businessRule").findMany({ where: { projectId } }),
...
async function getBusinessLogicSummary(projectId: string) {
  const [rulesCount, validationsCount, stateMachinesCount, policiesCount, processesCount, decisionTablesCount] = await Promise.all([
    safeDb("businessRule").count({ where: { projectId } }),
    safeDb("businessRule").count({ where: { projectId, ruleType: 'validation' } }),
    safeDb("stateMachine").count({ where: { projectId } }),
    safeDb("policy").count({ where: { projectId } }),
    safeDb("businessProcess").count({ where: { projectId } }),
    safeDb("decisionTable").count({ where: { projectId } }),
  ]);
...
async function getBusinessLogicHealth(projectId: string) {
  ...
  const conflicts = await safeDb("ruleConflict").count({ where: { projectId, status: 'open' } });
  const failedTests = await db.ruleTestResult.count({ where: { projectId, passed: false } });
  const recentExecutions = await safeDb("ruleExecutionLog").count({
