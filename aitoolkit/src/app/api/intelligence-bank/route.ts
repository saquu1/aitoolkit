...
async function getField(fieldId: string) {
  ...
  const field = await db.unifiedField.findUnique({
    where: { id: fieldId },
    include: {
      UnifiedFieldValidation: true,
      UnifiedFieldTestCase: true,
      UnifiedFieldSOPCompliance: true,
      UnifiedFieldCSHTMLEvidence: true,
      UnifiedFieldSPEvidence: true,
      UnifiedFieldDocumentation: true,
    },
  });
...
async function getSOPViolations(projectId: string) {
  ...
  return NextResponse.json({
    ...
    violations: violations.map(v => ({
      fieldId: v.fieldId,
      fieldName: v.UnifiedField.fieldName,
      tableName: v.UnifiedField.tableName,
      sopId: v.sopId,
      sopName: v.sopName,
...
async function runAllSOPAutoFixes(projectId: string) {
  ...
  for (const violation of violations) {
    try {
      ...
      const field = violation.UnifiedField;
      const updates: Record<string, unknown> = {};
      const appliedChanges: string[] = [];
